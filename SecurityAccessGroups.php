...
class SecurityAccessGroups extends AbstractExternalModule
{
            ...
            function checkImportErrors() {
                if (window.import_type) {
                    ...
                    if (window.import_type == "users") {
                        ...
                        text =
                            `The following users included in the provided import file cannot have the following permissions granted to them:<br><table style="margin-top: 20px; width: 100%;"><thead style="border-bottom: 2px solid #666;"><tr><th>User</th><th>Permissions</th></tr></thead><tbody>`;
                        const users = Object.keys(window.import_errors);
                        users.forEach((user) => {
                            text +=
                                `<tr style="border-top: 1px solid #666;"><td>${user}</td><td>${window.import_errors[user].join('<br>')}</td></tr>`;
                        });
                        text += `</tbody></table>`;
                ...
                $.post('<?= $this->getUrl("ajax/edit_user.php?pid=$project_id") ?>', permissions, function(data) {
                    ...
                    try {
                        ...
                        if (!result.role) {
                            ...
                            text =
                                `The following permissions cannot be granted to that user:${createRightsTable(result.bad_rights[users[0]])}`;
                        } else {
                            title = `You cannot grant those rights to the role<br>"${result.role}"`;
                            text =
                                `The following users are assigned to that role, and the following permissions cannot be granted to them:<br><table style="margin-top: 20px; width: 100%;"><thead style="border-bottom: 2px solid #666;"><tr><th>User</th><th>Permissions</th></tr></thead><tbody>`;
                            users.forEach((user) => {
                                text +=
                    ...
                    if (data == 1) {
                        ...
                        $.post('<?= $this->getUrl("ajax/assign_user.php?pid=$project_id") ?>', {
                            ...
                            try {
                                ...
                                text =
                                    `The following permissions allowed in user role "${result.role}" cannot be granted to that user: ${result.bad_rights[users[0]].join(', ')}`;
    ...
    function isUserExpired($username, $project_id)
    {
        ...
        $result = $this->query($sql, [ $username, $project_id ]);
