...
.Users tab
image::cc_users_2.png[users, 600]

This tab allows admins to assign users to SAGs. Users can be assigned to SAGs individually or in bulk using the `Import User Assignments` feature (see <<_import_file_format>>).

.User assignment
image::cc_users_edit_2.png[users assign, 600]

.Users actions
image::cc_users_actions_2.png[users actions, 600]
...
|sag_id
|The SAG ID of the SAG to assign the user to. Can be found on the <<_security_access_groups_tab>> of the module.
|===
...
image::cc_user_import_confirm_2.png[user import confirm, 600]
