...
<div class="SUR_Container">
    <div id="sub-nav" class="d-none d-sm-block mr-4 mb-0 ml-0">
        <ul>
            <li class="<?= $tab === "userlist" ? "active" : "" ?>">
                <a href="<?= $module->getUrl('system-settings.php?tab=userlist') ?>"
            ...
            <li class="<?= $tab === "roles" ? "active" : "" ?>">
                <a href="<?= $module->getUrl('system-settings.php?tab=roles') ?>"
                    style="font-size:13px;color:#393733;padding:7px 9px;">
    ...
    function handleFiles() {
        ...
        reader.onload = (e) => {
            ...
            $.post("<?= $module->getUrl("ajax/importCsvUsers.php") ?>", {
    ...
    $(document).ready(function() {
        ...
        $('.roleSelect').change(function() {
            ...
            const url = '<?= $module->getUrl("ajax/setUserRole.php") ?>';
            let color = "#66ff99";
            $.post(url, {
                    "username": user,
                    "role": newRole
        ...
        const deleteRoleButtonCallback = function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.showLoading();
                    $.post("<?= $module->getUrl("ajax/deleteSystemRole.php") ?>", {
                            role_id: role_id
                        })
                        .done(function(response) {
                            Toast.fire({
            ...
            Swal.fire({
                ...
                .then(function(result) {
                    if (result.isConfirmed) {
                        ...
                        data.newRole = 1;
                        $.post('<?= $module->getUrl("ajax/editSystemRole.php") ?>', data)
    ...
    function editRole(role_id, role_name) {
        const url = "<?= $module->getUrl("ajax/editSystemRole.php?newRole=false") ?>";
    ...
    function addNewRole() {
        $('#addRoleButton').blur();
        const url = "<?= $module->getUrl("ajax/editSystemRole.php?newRole=true") ?>";
    ...
    function handleFiles() {
        ...
        const reader = new FileReader();
        reader.onload = (e) => {
            $.post("<?= $module->getUrl("ajax/importCsvRoles.php") ?>", {
