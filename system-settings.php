        ...
        <script>
            window.sags = {
                <?php foreach ( $sags as $sag ) {
                    echo "'" . $sag["role_id"] . "': '" . $sag["role_name"] . "',";
                } ?>
            };
                    ...
                    format: {
                        ...
                        body: function (html, row, col, node) {
                            if (col === 0) {
                                ...
                                return window.sags[allData[row]["system_role"]];
                            } else if (col === 4) {
                ...
                const dt = $('#SUR-System-Table').DataTable({
                    ...
                    columns: [{
                        ...
                        data: function (row, type, set, meta) {
                            if (row.system_role === null) {
                                row.system_role = '<?= $module->defaultSagId ?>';
                            }
                            if (type === 'filter') {
                                return row.system_role + ' ' + window.sags[row.system_role];
                            } else if (type === 'sort') {
                                return window.sags[row.system_role];
                            } else {
                                let result =
                                    `<select class="sagSelect" disabled="true" onchange="saveSag(this)">`;
                                for (let sag_id in sags) {
                                    const sag_label = sags[sag_id];
                                    const selected = sag_id == row.system_role ?
                                        "selected" : "";
                                    result +=
                    ...
                    columnDefs: [{
                        ...
                        createdCell: function (td, cellData, rowData, row, col) {
                            $(td).attr('data-sag', rowData.system_role);
                        }
                    }, {
                        targets: [4],
                        visible: false,
        ...
        <script>
            ...
            function openSagEditor(url, sag_id = "", sag_name = "") {
                ...
                const saveSagChangesButtonCallback = function () {
                    ...
                    const sag_name_edit = $('input[name="role_name_edit"]').val();
                    if (sag_name_edit != '') {
                        const data = $("#SAG_Setting").serializeObject();
                ...
                $.get(url, {
                    ...
                    .done(function (response) {
                        ...
                        $("#edit_sag_popup").on('shown.bs.modal', function (event) {
                            ...
                            $('#SAG_Save').click(role_id == "" ? saveNewSagButtonCallback :
            ...
            function editSag(sag_id, sag_name) {
                const url = "<?= $module->framework->getUrl("ajax/editSag.php?newRole=false") ?>";
                openSagEditor(url, sag_id, sag_name);
            }

            function addNewSag() {
                $('#addSagButton').blur();
                const url = "<?= $module->framework->getUrl("ajax/editSag.php?newRole=true") ?>";
                        ...
                        data: function (row, type, set, meta) {
                            if (type === 'display') {
                                ...
                                return `<div style="display: flex; align-items: center; white-space: nowrap;">` +
                                    `<i class="${iclass}"></i>` +
                                    `<a class="${aclass}" onclick="editSag('${row.role_id}')">${row.role_name}</a>` +
                                    `</div>`;
                            } else {
