...
class Alerts
{
    ...
    public function getAlerts($project_id = null)
    {
        ...
        $sql    = "SELECT log_id, timestamp, message 'Type', user, users, alertType 'Alert Type', recipient, recipientAddress, reminderDate, fromEmail WHERE message IN ('ALERT', 'REMINDER') AND project_id = ?";
        $params = [ $project_id ];
        ...
        while ( $row = $result->fetch_assoc() ) {
            ...
            $thisAlert["timestamp"]    = $row["timestamp"];
            $thisAlert["type"]         = \REDCap::escapeHtml($row["Type"]);
            $thisAlert["alertType"]    = \REDCap::escapeHtml($row["Alert Type"]);
            $thisAlert["to"]           = \REDCap::escapeHtml($row["recipientAddress"]);
            $thisAlert["from"]         = \REDCap::escapeHtml($row["fromEmail"]);
            $thisAlert["reminderDate"] = \REDCap::escapeHtml($row["reminderDate"]);
